...
vi.mock("@/features/chat/components/chat-composer", { spy: true });
vi.mock("@/features/chat/components/chat-conversation", { spy: true });
vi.mock("@/features/chat/components/chat-header-client", { spy: true });
vi.mock("@/features/workspace/components/chat-preview", { spy: true });
vi.mock("@/features/chat/chat-query-options", () => ({
...
vi.mock("@/features/workspace/components/chat-code", () => ({
...
beforeEach(() => {
  // Model Next's subscription to native history writes.
...
async function renderPage() {
  await renderWithProviders(
    <ChatWorkspace
      key={threadId}
      thread={{ isStreaming: false, messages: [], resourceId: threadId }}
      threadId={threadId}
    />
  );
  await flushChatUpdates();
...
it("follows URL changes without losing the draft or unrelated URL state", async () => {
  window.history.replaceState(
    null,
    "",
    "/?view=code&file=src%2FApp.tsx#workspace"
  );
  await renderPage();
  ...
  await act(() => window.history.replaceState(null, "", "/?view=code"));
  await flushChatUpdates();
  expect(
    container.querySelector('[role="tabpanel"]:not([hidden])')?.textContent
  ).toBe("Workspace files");
...
it("starts from server history and sends the server-verified owner with the next turn", async () => {
  await renderWithProviders(
    <ChatWorkspace
      thread={{
        isStreaming: false,
        messages: [
          {
            id: "stored-user",
            parts: [{ text: "Stored prompt", type: "text" }],
            role: "user",
          },
        ],
        resourceId: "signed-in-owner",
      }}
      threadId={threadId}
    />
  );
  ...
  await enterMessage("Next turn");
  await clickButton("Send Message");
  expect((await getChatRequest(0).json()).memory).toEqual({
    resource: "signed-in-owner",
    thread: threadId,
  });
});
...
it("isolates draft edits and streamed deltas from layout and completed messages", async () => {
  ...
  vi.mocked(ChatHeaderClient).mockClear();
  ...
  expect(ChatHeaderClient).not.toHaveBeenCalled();

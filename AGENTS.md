...
- Document each function with a summary, `@param options`, `@returns`, and a usage `@example`; include `@throws` for expected failure cases
- Declare explicit return types and keep parameter defaults in destructuring; default the options object to `{}` only when all properties are optional
- Update all callers and tests when changing function signatures
